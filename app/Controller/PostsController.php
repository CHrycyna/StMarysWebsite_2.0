...
class PostsController extends AppController {
		...
		if($this->request->is('post'))
		{
			if(isset($this->params['data']['username']))
			{
				...
				$posts = $this->Post->find('all', array(
						'conditions' => array('User.username' => $this->params['data']['username']), //array of conditions
						'fields' => array('Post.*', "User.username"),
						'joins' => array(
								array('table' => 'users',
										'alias' => 'User',
	...
	public function api_tag() {
		...
		if($this->request->is('post'))
		{
			if(isset($this->params['data']['tagName']))
			{
				$this->loadModel('PostTag');
				$this->loadModel('Tag');
				$this->loadModel('User');
				...
				$posts = $this->Post->find('all', array(
						'order' => array('Post.created DESC'),
						'fields' => array('Post.*', 'User.username'),
						'conditions' => array('Tag.tag' => $this->params['data']['tagName']), //array of conditions
						'joins' => array(
								array(	'table' => 'users',
										'alias' => 'User',
										'type' => 'INNER',
